  ...
  methods: {
    ...
    handleSubjectAreas() {
      ...
      myChart.on("click", function (params) {
        ...
        that.axios
          .get("/cooperativeProject/internationalPartners/subSubject", {
            params: {
              subject: name2,
              continent: that.subjectData.continent,
              cooperationCountry: that.subjectData.cooperationCountry,
              cooperationCity: that.subjectData.cooperationCity,
              cooperationInst: that.subjectData.cooperationInst,
              executeState: that.subjectData.executeState,
              executeYear: that.subjectData.executeYear,
              instName: that.subjectData.instName,
              limit: that.subjectData.limit,
              offset: that.subjectData.offset,
              projectClass: that.subjectData.projectClass,
              year: that.subjectData.year
            },
          })
          .then((res) => {
            if (res.data.code == 200) {
              var subSubject = res.data.data.subjectMap.SecondSubject[0].value;
              if (seriesData.length >= 2) {
