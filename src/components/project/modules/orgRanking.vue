<template>
  <div class="subsidize">
    <div class="indexTitleDiv">
      <div class="titleName">合作项目资助金额全院机构排名</div>
      <div class="operating">
    ...
    <div class="subsidizeCharts" v-show="isShowChart">
      <div class="subsidizeChart" ref="mySubsidizeChart">
        ...
        <div class="rankCon" :style="{height:rankConrHeight}">
          <div ref="rankConBoxDiv">
            ...
            <div class="conItem" v-for="(item,index) in foundingData.table" :key="index">
              <div class="conLeft">{{item.instName}}</div>
              <div class="conRight">
                <div  class="colorDiv" :style="{width:((Number(item.value)/Number(rankRange[6])*(13/14))*100+'%') , marginLeft:((rangeItemWidth/2)+'px')}"></div>
                <div class="textDiv">{{Math.ceil(item.value)}}</div>
              </div>
            </div>
          </div>
    ...
    <div id="subsidizeList" v-show="!isShowChart">
      <table>
        <thead>
        <tr>
          <th style="width: 70%;text-align: center">机构名称</th>
          <th style="width: 30%;">人次</th>
        </tr>
        </thead>
        <tbody v-if="foundingData != undefined">
        <tr v-for="(item,i) in foundingData.table" :key="i">
          <td style="width: 70%;text-align: center">{{item.instName}}</td>
    ...
    <transition name="bounce" mode="out-in">
      <div class="fullFcreen" v-if="!change">
        ...
        <div class="subsidizeCharts" v-show="isShowChart">
          <div class="subsidizeChart" ref="mySubsidizeChart mySubsidizeChart2">
            <div class="rankTitle">
              <div class="rangeLeftDiv"></div>
        ...
        <div id="subsidizeList" v-show="!isShowChart">
          <table>
            <thead>
            <tr>
              <th style="width: 70%;text-align: center;text-indent: 100px">机构名称</th>
              <th style="width: 30%;">人次</th>
            </tr>
            </thead>
            <tbody v-if="foundingData != undefined">
  ...
  export default {
    data() {
      return {
        ...
        isShowChart: true,
        tabNum: "",
        rankRange:['0','20000','40000','60000','80000','100000','120000','140000'],
        maxNum:'140000',
    ...
    watch: {
      data: {
        ...
        handler (val) {
          ...
          if(val.table){
            let arr = visitFun.getRangeType(val.table[0].value)

            this.rankRange = arr

            let myChart = this.$echarts.init(this.$refs.mySubsidizeChartHover)
            myChart.hideLoading();
          }
      ...
      change: {
        ...
        handler (val) {
          let that = this
          if(val == true){
            if(this.$refs.rangeItemRef2 != undefined) {
            this.rangeItemWidth2 = (this.$refs.rangeItemRef2.clientWidth)/7
            }
            if(this.$refs.rankConBoxDiv2 != undefined) {
    ...
    mounted() {
      ...
      myChart.showLoading();
      // setTimeout(function () {
      //   let myChart = that.$echarts.init(that.$refs.mySubsidizeChartHover)
      //   myChart.hideLoading();
      // },3000)
    },
    methods: {
