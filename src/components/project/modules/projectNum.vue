  ...
  methods: {
    ...
    handleSubsidizeChart() {
      let myChart = this.$echarts.init(this.$refs.mySubsidizeChart),
        _this = this;
      myChart.showLoading();
      if (this.foundingData.projectNumber != undefined) {
        var datas = this.foundingData.projectNumber;
        // var year = this.foundingData.projectNumber.year

        let option = {
          tooltip: {
            trigger: "axis",
            formatter:function(params) {
              return params[0].name + '年合作项目数：' + params[0].data
            }
          },
          grid: {
            left: "3%",
