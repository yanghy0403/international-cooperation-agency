<template>
  <div class="CooperationTrackCon">
    <div class="titleDiv">
      <div class="titleName">与中科院合作轨迹</div>
      <div class="operating">
...
export default {
  ...
  watch:{
    linksData(val){
      ...
      this.linksData2 = [...this.linksData2 , ...val]
  ...
  mounted() {
    ...
    this.tableData2 = this.detailData



    if(this.linksData.length != 0){
      this.linksData.forEach((item,index)=>{
        let newSource = item.source
        item.source = item.target
        item.target = newSource
        item.isReal = true
    ...
    this.linksData2.forEach((item,index)=>{
      this.linksData.forEach((subItem,subIndex)=>{
        if(item.source == subItem.source && item.target == subItem.target){
          this.linksData2.splice(index,1)
        }
      })
    ...
    this.linksData2 = [...this.linksData2 , ...this.linksData]
    this.handleCooperationTrack();
    this.$nextTick(() => {
  ...
  methods: {
    ...
    handleCooperationTrack() {
      ...
      let bagColor = ['#dbcfb4','#b5937b','#d79494','#a698b8','#7a779c',
        '#89a4b3','#6a7f9e','#475c7b','#bfcea5','#80bdc4','#edb399','#d3be92','#84a0c9','#e18787',]
      let option = {
        color:bagColor,
        series: {
          type: "sankey",
