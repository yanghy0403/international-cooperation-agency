    ...
    <div class="workExperience2" v-else>
      <div class="tabDIv newTable2 newScrollbargray">
        <div v-if="internationtableDataArr.length != 0">
          <div class="tabletitle">国际伙伴计划</div>
          <el-table
        ...
        <div v-if="taiWantableDataArr.length != 0">
          <div class="tabletitle">科技部国际合作项目</div>
          <el-table
    ...
    watch:{
      projectData:{
        ...
        handler (val) {
          ...
          //    处理table数据
          if(this.newworkData.table.InternationalPartnerList){
            this.newworkData.table.InternationalPartnerList.forEach((item,index)=>{
          ...
          if(this.newworkData.table.ScienceDepartmentCooperationList){
            this.newworkData.table.ScienceDepartmentCooperationList.forEach((item,index)=>{
