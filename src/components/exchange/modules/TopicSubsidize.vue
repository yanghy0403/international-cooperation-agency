<template>
  <div class="subsidize">
    <div v-if="change">
      ...
      <div style="overflow-x: scroll" ref="myTableImg"  id="subsidizeList" v-show="!isShowChart">
    ...
    watch: {
      data: {
        ...
        handler (val) {
          ...
          // }
          if(this.visitType == '出访'){
            this.tableTitle = ['年份','考察访问','海峡两岸会议','国际会议','商务活动','合作研究','参加培训','其它','总计',]
          }else if(this.visitType == '来访'){
            this.tableTitle = ['年份','海峡两岸会议','学术访问','国际会议','合作研究','参加培训','其它','总计',]
          }

          this.componentTitle = `历年${this.visitType}人次`
    ...
    methods: {
      ...
      handleSubsidizeChart() {
        ...
        if(JSON.stringify(ydata) != "[]"){
          let length1 = ydata[0].length  //1
          let length2 = ydata.length    //6
        ...
        if (newVal != undefined) {
          ...
          //   if (newVal.year != undefined) {
              ...
              for (var j = 0; j < newVal.xAxisData.length; j++) {
                series.push({
                  ...
                  label: {
                    normal: {
                      ...
                      formatter: function (a, b) {
                        if(a.seriesName == _this.tailEmptyName){
        ...
        _this.myChart.off("legendselectchanged");
        _this.myChart.on('legendselectchanged', function (obj) {
          let selected = obj.selected ,
              name = obj.name
          let typeName = _this.foundingData.countByYear.xAxisData
          let arr = []
          _this.totalArr = []
          ...
          arr.forEach((item,index)=>{
            if(item == 1){
              _this.tailEmptyName = typeName[index]
            }
          })
          if(JSON.stringify(ydata) != "[]"){
            let length1 = ydata[0].length  //1
