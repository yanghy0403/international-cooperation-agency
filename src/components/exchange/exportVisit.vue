  ...
  <div class="exchangeCon">
    <div class="ex-left">
      ...
      <div class="ex-nav">
        ...
        <div class="navActive"><router-link tag="li" to="/exportVisit">来访</router-link></div>
        <div><router-link tag="li" to="/mutualVisits">总计</router-link></div>
        ...
        <div class="fr-reel-left">
          <div class="fr-reelHeader">
            ...
            <div class="searchDiv" style="">
              ...
              <div @click="wordDownload" style="cursor: pointer" :style="{background:conditionType?'#ff3a25':'#f1f3f6' , color:conditionType?'#fff':'#727272'}"  class="reel-DownloadAlls reel-DownloadAllsDetail iconfont icon-xiazai"></div>
            </div>
          ...
          <div class="fr-reelCenter newScrollbar">
            <div class="reelCentent  " >
              <div class="moduleDiv" style="width: 710px;height: 390px;" ref="picture1">
                <TopicSubsidize
                  :data="TopicFounding"
                  :mode="mode"
                  visitType="来访"
              ...
              <div class="moduleDiv" style="width: 710px;height: 390px;" ref="picture2">
                <TopicSubjectAreas
                  :data="TopicSubject"
                  :mode="mode"
                  :visitType="visitType"
            ...
            <div class="reelCentent" style="margin-top: 10px;">
              <div class="moduleDiv" style="width: 710px;height: 390px;"  ref="picture3">
                <personRanking
              ...
              <div class="moduleDiv" style="width: 710px;height: 390px;"  ref="picture4">
                <orgRanking
                  :data="orgRankingData"
                  :visitType="visitType"
                  :mode="mode"
            ...
            <div class="reelBottom" style="width: 1430px; height: 430px;"  ref="picture5">
              <IdentityGlobalMap
    ...
    <div  class="ex-right2" >
      ...
      <div class="personList" >
        ...
        <div class=" personHeaderNew" style="display: flex">
          <h3 class="personTitleName">来访</h3>
          <div>
        ...
        <div v-show="isListShow" class="arrayList" v-loading="listLoading" element-loading-text="加载中...">
          <ul>
            <li v-for="(item,i) in TopicpersonList.personList" :key="i" >
              <!-- 头像 -->
              <div @click="toPersonDetail(item.uniqueId)" class="avatar">
                <img v-if="item.imgUrl" :src="url + item.imgUrl" alt />
                <img v-else src="../../assets/images/国际人才/默认头像无头像状态.png" alt />
              </div>
              <!--来访：  姓名、国籍、职称、学科领域、访问单位、工作单位 -->

              <!-- 名字 -->
              <div
                class="personName"
                @click="toPersonDetail(item.uniqueId)"
              >{{item.name}}</div>
              <!-- 国家 -->
              <div class="personCountry">{{item.teamState}}</div>
              <div class="line"></div>
              <!-- 职称 - 学科 -->
              <div
                class="personSpecialty"
              >{{item.personTitle}}·{{item.personSubject}}</div>
              ...
              <div class="ENunit">
                <label>工作单位：</label>
                <span>{{item.unit?item.unit:'暂无数据'}}</span>
              </div>
            </li>
          </ul>
        </div>
        <div v-show="!isListShow" class=" globaltableScroll tableList" v-loading="listLoading" element-loading-text="加载中...">
          <Table
            :height="tableHeight"
            border
            :columns="columns1"
  ...
  export default {
    ...
    data(){
      return{
        ...
        IdentityGlobalData:{},
        InstituteImportData:{},
        TopicpersonList:TopicpersonList,
        orgAll:[],
        countriesAll:[],
        cityAll:[],
        foreignOrgAll:[ ],
    ...
    methods:{
      ...
      //二级搜索
      secondLevelSearch(){
        // this.getImportVisitData()
        this.getvisitOrgData()
        this.getvisitedOrgData()
        this.getperNumData()
        this.gettableDataData()
      ...
      * */
      excalDownload(){
        let params = {
          ...
          city:'',
          enOrg:this.params2.foreignOrgSelect,
          zhOrg:this.params2.orgSelect,
          subject:this.params2.subjectSelect,
          year:this.params2.yearSelect,
        }
        this.axios(visitFun.settingParameters('post','/communication/export',params)).then((res) => {
          if (res.data.code == 200) {
      ...
      * */
      ...
      getvisitOrgData() {
        ...
        this.axios.get("/communication/visit/visitOrg", {
          params: {
          ...
          if (res.data.code == 200) {
            this.personRankingData = res.data.data.visitOrg
      ...
      getvisitedOrgData() {
        ...
        this.axios.get("/communication/visit/visitedOrg", {
          params: {
            city: this.params.citySelect,   //城市
            continent: this.params.continentSelect,  //大洲
            country: this.params.countrySelect,  //国家
          ...
          if (res.data.code == 200) {
            res.data.data.visitedOrg.chartData.splice(20,40)

            this.orgRankingData = res.data.data.visitedOrg
      ...
      getperNumData() {
        ...
        this.axios.get("/communication/visit/perNum", {
          params: {
            city: this.params.citySelect,   //城市
            continent: this.params.continentSelect,  //大洲
            country: this.params.countrySelect,  //国家
          ...
          if (res.data.code == 200) {
            this.TopicFounding.countByYear = res.data.data.countByYear
      ...
      // 历年出访人次table
      gettableDataData() {
        this.axios.get("/communication/visit/tableData", {
          params: {
            city: this.params.citySelect,   //城市
            continent: this.params.continentSelect,  //大洲
            country: this.params.countrySelect,  //国家
          ...
          if (res.data.code == 200) {
            this.TopicFounding.tableData = res.data.data.tableData
          }
        });
      },
      ...
      getsubjectDetailData() {
        ...
        this.axios.get("/communication/visit/subjectDetail", {
          params: {
            city: this.params.citySelect,   //城市
            continent: this.params.continentSelect,  //大洲
            country: this.params.countrySelect,  //国家
      ...
      getlistData() {
        ...
        this.axios.get("/communication/visitList", {
          params: newParams,
        }).then((res) => {
          if (res.data.code == 200) {
            this.TopicpersonList = res.data.data
            this.listLoading = false
            if(this.TopicpersonList){
              for(let item in this.TopicpersonList.personList[0]){
      ...
      getMapData() {
        ...
        this.axios.get("/communication/globalMap", {
          params: {
            city: this.params.citySelect,   //城市
            continent: this.params.continentSelect,  //大洲
            country: this.params.countrySelect,  //国家
          ...
          if (res.data.code == 200) {
            let newObject = visitFun.correctLocation(res.data.data.geoCoordMap)
            this.IdentityGlobalData = {
              geoCoordMap: newObject,
              continentData: res.data.data.continent,
              data: res.data.data.count,
              normalize: res.data.data.normalize,
              city: this.params.citySelect,   //城市
              continent: this.params.continentSelect,  //大洲
              country: this.params.countrySelect,  //国家
              enOrg: this.params.foreignOrgSelect,        //国外机构
      ...
      getInstituteMapData() {
        this.mapLoading = true
        // type: '来访',
        this.axios.get("/communication/globalMap/zky", {
          params: {
            city: this.params.citySelect,   //城市
            continent: this.params.continentSelect,  //大洲
            country: this.params.countrySelect,  //国家
          ...
          if (res.data.code == 200) {
            ...
            //   cityCount:res.data.data.cityCount,
            //   cityLongAndLat:[...res.data.data.city[0]],
            ...
            this.InstituteImportData.data.org = res.data.data.org
            this.InstituteImportData.data.cityCount = res.data.data.cityCount
            this.InstituteImportData.data.cityLongAndLat.push(res.data.data.city[0])
            this.mapLoading = false
          }
        });
      },
